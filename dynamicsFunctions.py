...
max_traction_force = mass * g * longitudinal_friction   # N - max force in LONGITUDINAL DIRECTION
...
def fastestNextSpeed(dataDict, i):
    ...
    poly_coeffs = np.array([1/2, dataDict['v0'][i], -delta_d])          # coefficients: p[0] * x^n + ... + p[n]
    dt = np.roots(poly_coeffs)[1]                                       # This is the delta_t
    dataDict['t0'][i+1] = dataDict['t0'][i] + dt

    # then for v1
    dataDict['v0'][i+1] = dataDict['v0'][i] + dataDict['a_tan0'] * dt
